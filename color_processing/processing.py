...
def rgb2gray(rgb_image, width_image, height_image):
    # Definition of necessary variables 
    rows_device = round(width_image / 100)
    columns_device = round(height_image / 100)

    # Creating vectors for processing
    rgb_image_host = np.array(rgb_image).astype(np.uint32)
    gray_image_host = np.zeros((height_image, width_image)).astype(np.uint32)

    rgb_image_device, gray_image_device = copy_host_to_device(rgb_image_host, gray_image_host)
    ...
    # Copy device variable to host device
    cuda.memcpy_dtoh(gray_image_host, gray_image_device)
    return gray_image_host

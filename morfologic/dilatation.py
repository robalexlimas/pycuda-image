...
def dilatation(binimage, height_image, width_image, filter, height_filter, width_filter):
    ...
    dilatation_cpu = np.zeros((height_image, width_image)).astype(np.uint32)
    ...
    dilatation_gpu = cuda.mem_alloc(dilatation_cpu.nbytes)
    ...
    dilatation = mod.get_function("dilatation")
    ...
    cuda.memcpy_dtoh(dilatation_cpu, dilatation_gpu)
    return dilatation_cpu